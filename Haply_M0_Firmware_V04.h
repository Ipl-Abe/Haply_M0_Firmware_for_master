...
float vir_M1torque=0.0;
float vir_M2torque=0.0;

float vir_torques[TOTAL_ACTUATOR_PORTS];
...
char write_states(pwm pwmPins[], actuator actuators[]){
  ...
  for(int i = 0; i < TOTAL_ACTUATOR_PORTS; i++){
    ...
    if(actuators[i].hostAddress == deviceAddress){
      ...
      actuators[i].torque = vir_torques[i] + torque_temp;
