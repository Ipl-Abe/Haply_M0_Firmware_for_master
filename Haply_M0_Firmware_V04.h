...
bool create_angle(actuator *mtr, float angle_value, float ref_angle);
...
char write_states(pwm pwmPins[], actuator actuators[]){
  ...
  for(int i = 0; i < TOTAL_ACTUATOR_PORTS; i++){
    ...
    if(actuators[i].hostAddress == deviceAddress){
      ...
      actuators[i].torque = BytesToFloat(segments);
...
bool create_angle(actuator *mtr, float angle_value, float ref_angle){
  ...
  // calculate duty corresponds to the angle difference
    ...
    case 9:
      ...
      res = (M1_ref_angle - angle_value)/15.0;
      Motor_duty = (-1 / (res*res+1) +1 ) * 650;
      
      if(angle_res > 2){
        mtr->rotationalDirection = 1;
        pwm_write_duty(mtr->pwmPin, Motor_duty);
//        status=false;
      }
      else if(angle_res < -2){
        mtr->rotationalDirection = 0;
        pwm_write_duty(mtr->pwmPin, Motor_duty);
//        status=false;
      }
      else{
        pwm_write_duty(mtr->pwmPin, 0);
        status = true;
      }
      break;
    case 12:
      ...
      res = (M2_ref_angle - angle_value)/15.0;
      Motor_duty = (-1 / (res*res+1) +1 ) * 650;
      if(angle_res > 2){
        mtr->rotationalDirection = 0;
        pwm_write_duty(mtr->pwmPin, Motor_duty);
