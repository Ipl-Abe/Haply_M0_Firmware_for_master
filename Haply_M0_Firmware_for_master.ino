  ...
  if(currentState - lastPublished >= 50){
    ...
    if(SerialUSB.available() > 0){
      ...
      switch(cmdCode){
        ...
        // Debug 1 : send torque_data or angle data to Arduino UNO
          Serial1.write('+');
          Serial1.write(M1_angle_data, 4);
          Serial1.write(M2_angle_data, 4);
          
          // get torque data from slave Haply device and refrect them
          inChar = (char)Serial1.read();
          if(inChar == '+'){
            Serial1.readBytes(M1_torque_char,4);
            Serial1.readBytes(M2_torque_char,4);
            M1_ref_torque= BytesToFloat(M1_torque_char);
            create_torque(&actuators[0], M1_ref_torque );
            M2_ref_torque= BytesToFloat(M2_torque_char);
            create_torque(&actuators[1], M2_ref_torque );
          }
